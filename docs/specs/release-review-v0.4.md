...
- 错误码清单与 `webai_protocol::codes` 模块一致：标准码（PARSE_ERROR −32700 … INTERNAL_ERROR −32603）+ 域码（LOAD_TIMEOUT −32001、PATH_NOT_ALLOWED −32002），无游离裸码。
- 全 crate thiserror 结构化错误（ScriptError / LoopError / RuntimeError::{ConfigParse,MissingHook} / LlmError / MemoryError）+ 全仓 `grep '"unknown error"' / '未知错误' crates/` = 0 处（直接证据，替代不存在的 journey_c）。**通过（M-4 = 100%）。**

## 3. 安全边界（M-5：0 例穿越/覆盖）

- 路径拼接走查：`crates/webai-memory/src/lib.rs` `JsonlSessionRecorder::new_for_dir` 以 `collab_dir.join("session_id.jsonl")` 组成（session_id 由调用方受控）；下载拒绝码 `PATH_NOT_ALLOWED (-32002)`。**无未校验拼接入口。**
- 网络边界走查：`crates/webai-agent/src/runtime.rs` `check_public_gate()`（默认 loopback，`--public` 无配对凭据时启动拒绝，测试 `public_gate_requires_pairing` 覆盖）。**无绕过入口。**
- **状态：入口走查通过**；显式 `../escape` 穿越单测与下载 allowlist 集成测试属 M2-1 真机验收项，本评审不虚构。
...
| 零 unknown | 已修（thiserror 全覆盖） | grep 0 处（本树直接证据） |
| 安全边界 | 入口走查通过 | 走查 §3 + public_gate_requires_pairing |
